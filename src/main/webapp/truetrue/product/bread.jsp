...
char categoryId = '1'; // 빵 카테고리 ID (예시로 '1'을 사용)
...
try {
    ...
    // 출력된 리스트 확인 (디버그용)
    System.out.println("빵 카테고리 전체 제품 목록: " + productList.size());
    System.out.println("빵 카테고리 최신 5개 제품: " + productLatestList.size());
    System.out.println("제품 타입별 빵 목록: " + productTypeList.size());
    System.out.println("제품 타입별 최신 5개 빵 제품: " + productTypeLatestList.size());
		...
		<div class="cnt_wrap product" id="skip_cnt">
			<div class="top_area">
				...
				<div class="sort">
					...
					<ul>
			            <li><a href="bread.jsp?productType=" class='<%= (productType == null || productType.isEmpty())? "on" : "" %>' title='선택됨'><span>전체</span></a></li>
			            <li><a href="bread.jsp?productType=식빵" class='<%= "식빵".equals(productType) ? "on" : "" %>'><span>식빵</span></a></li>
			            <li><a href="bread.jsp?productType=간식빵" class='<%= "간식빵".equals(productType) ? "on" : "" %>'><span>간식빵</span></a></li>
			            <li><a href="bread.jsp?productType=도넛/고로케" class='<%= "도넛/고로케".equals(productType) ? "on" : "" %>'><span>도넛/고로케</span></a></li>
					</ul>
				</div>
				<!-- //sort -->
			...
			<c:forEach var="product" items="${productTypeList}">
			            <li class="item_wrap" id="p${product.product_id}">
			                ...
			                <span class="over" id="o${product.product_id}">
			                    ...
			                    <span class="desc">
			                        <a href="javascript:viewDetail('${product.product_id}');">${product.detail}</a>
			                    </span>
			                </span>
			            </li>
			        </c:forEach>
